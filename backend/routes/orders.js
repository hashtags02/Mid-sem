...
router.get('/available/list', optionalAuth, async (req, res) => {
  try {
    if (mongoose.connection && mongoose.connection.readyState === 1) {
      const docs = await Order.find({ status: { $in: ['pending', 'confirmed', 'preparing', 'ready_for_pickup'] } }).sort({ createdAt: -1 }).lean();
      const mapped = docs.map(doc => ({
        id: doc.orderId,
        restaurantId: doc.restaurantId,
        restaurantName: doc.restaurantName,
    ...
    const available = Array.from(inMemoryOrders.values()).filter(o => ['pending', 'confirmed', 'preparing', 'ready_for_pickup'].includes(o.status));
  ...
  try {
    if (mongoose.connection && mongoose.connection.readyState === 1) {
      ...
      if (!['ready_for_pickup'].includes(doc.status)) {
        return res.status(400).json({ error: 'Order is not ready for assignment' });
      }
